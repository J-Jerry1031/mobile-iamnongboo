...
export function FullMenuButton() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('iamnongbu:close-menu', onClose);
    return () => {
      window.removeEventListener('iamnongbu:open-menu', onOpen);
      window.removeEventListener('iamnongbu:close-menu', onClose);
    };
  }, []);
  ...
  return (
    <button
      ...
      className={`flex flex-col items-center gap-1 text-[13px] text-[#111] transition-colors ${active ? 'font-black' : 'font-medium'}`}
      aria-label="전체 메뉴 열기"
      aria-pressed={active}
    >
      <Menu size={29} strokeWidth={active ? 2.1 : 1.7} />
      메뉴
    </button>
  );
}
...
export function FullMenuDrawer() {
  ...
  function closeMenu() {
    setOpen(false);
    window.dispatchEvent(new Event('iamnongbu:close-menu'));
  }
    ...
    <>
      {open && (
        <div className="fixed inset-0 z-[80]">
          <button type="button" aria-label="전체 메뉴 닫기" onClick={closeMenu} className="absolute inset-0 bg-black/35" />
          <aside className="absolute inset-y-0 left-1/2 flex w-full max-w-[430px] -translate-x-1/2 justify-end overflow-hidden pointer-events-none">
            <div className="pointer-events-auto h-full w-[86%] max-w-[370px] animate-[menuSlideIn_.22s_ease-out] overflow-y-auto bg-white px-5 pb-8 pt-[max(22px,env(safe-area-inset-top))] shadow-[-20px_0_40px_rgba(0,0,0,.18)]">
              <div className="flex items-start justify-between gap-4">
                ...
                <button type="button" onClick={closeMenu} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#f1ead9] text-[#214b36]" aria-label="전체 메뉴 닫기">
                  <X size={20} />
                </button>
              </div>

              <div className="mt-5 grid grid-cols-2 gap-2">
                <Link href="/mypage" onClick={closeMenu} className="rounded-2xl bg-[#214b36] p-4 text-white active:scale-[.99]">
                  <UserRound size={20} />
                ...
                <Link href="/products/market?sort=new" onClick={closeMenu} className="rounded-2xl bg-[#e5f0dc] p-4 text-[#214b36] active:scale-[.99]">
              ...
              <section className="mt-6">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {categoryLinks.map((item) => (
                    <MenuLink key={item.label} {...item} onClick={closeMenu} />
                  ))}
                </div>
              </section>
