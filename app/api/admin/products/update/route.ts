...
import { asRecord, safeCuid, safeInt, safeOptionalText, safeProductCategory, safeText, safeUrl } from '@/lib/security';
...
export async function PATCH(req: Request) {
  ...
  const stock = safeInt(body.stock, -1, 0, 100_000);
  ...
  const product = await prisma.product.update({
    ...
    data: {
      ...
      detailImage: safeUrl(body.detailImage, '') || null,
      origin: safeOptionalText(body.origin, 80),
      weight: safeOptionalText(body.weight, 80),
      storage: safeOptionalText(body.storage, 120),
      expiration: safeOptionalText(body.expiration, 120),
      allergy: safeOptionalText(body.allergy, 200),
